...
本指南说明如何将 Remote Control Server (RCS) 部署到私有环境，并通过 Agent Terminal CLI 连接使用。

## 架构概览

```
┌──────────────────┐                    ┌──────────────────────┐
│  Agent Terminal CLI  │ ◄── HTTP/SSE/WS ─►│  Remote Control      │
...
- 接收 Agent Terminal CLI 的环境注册和工作轮询
...
- 一台可被 Agent Terminal CLI 和 Web 浏览器同时访问的服务器（物理机、VM、容器均可）
- [Docker](https://www.docker.com/)
- 启用 `BRIDGE_MODE` feature flag 的 Agent Terminal 构建
...
## Agent Terminal 客户端连接

### 1. 设置环境变量

在运行 Agent Terminal 的机器上设置：
...
ACP session 在 Web UI 中显示品牌色标签，与普通 Agent Terminal session 区分。
...
└──────────────────────────────────────────────────────────┘
 ...
 1. Agent Terminal CLI 启动，设置环境变量指向自托管 RCS
