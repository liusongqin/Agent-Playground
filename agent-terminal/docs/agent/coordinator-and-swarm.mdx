...
description: "从源码角度解析 Agent Terminal 多 Agent 协作：Coordinator Mode 的 System Prompt 设计、Worker 生命周期、Task 通信协议和 Swarm 蜂群的任务分配机制。"
...
| 组件 | 角色 |
|------|------|
| **Team Lead** | 创建团队、分配任务、综合结果的主 Agent Terminal 会话 |
| **Teammate** | 独立的 Agent Terminal 实例，各自拥有独立的上下文窗口 |
