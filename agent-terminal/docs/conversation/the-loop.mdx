...
description: "深入解析 Agent Terminal 的 query() 异步生成器循环——从流式 API 调用、工具并行执行、上下文压缩、错误恢复到终止条件的完整状态机，基于 src/query.ts 的源码级分析。"
...
传统聊天机器人：你问一句，它答一句。  
Agent Terminal 不一样：你说一个需求，它可能连续执行十几步操作才给你最终结果。
...
<Note>
源码揭示了为什么 Agent Terminal 选择逐步循环：
</Note>
