...
description: "揭秘 Agent Terminal 如何通过 GrowthBook 实现运行时 A/B 测试：用户定向、tengu 命名文化和渐进式功能发布策略。"
...
所有运行时 flag 都以 `tengu_` 为前缀——"Tengu"（天狗）是 Agent Terminal 的内部项目代号。flag 名采用**动物/植物/矿物 + 形容词**的命名约定，刻意保持不透明。
