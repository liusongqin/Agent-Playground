...
description: "从源码角度解析 Agent Terminal 的 MCP 集成：内置 MCP 与外部 MCP 的区别、7 种传输层实现、connectToServer 的 memoize 缓存、工具发现的 LRU 策略、认证状态机、以及 MCP 工具如何进入权限检查链路。"
...
Agent Terminal 的 MCP 实现区分 **内置 MCP 服务器** 和 **外部 MCP 服务器**。两者使用相同的客户端协议和工具发现机制，但在连接方式、生命周期管理和配置来源上完全不同。

### 内置 MCP 服务器

内置 MCP 服务器由 Agent Terminal 自身提供，无需用户手动配置。它们在启动时自动注册为 `dynamic` scope 的配置，并在同进程内运行。
...
VSCode SDK MCP 是特殊的内置模式。IDE（如 VS Code、JetBrains）通过嵌入方式启动 Agent Terminal，并传入 `type:'sdk'` 的 MCP 配置。这类配置：
...
`fetchToolsForClient()`（`client.ts:1744-2000`）使用 `memoizeWithLRU` 缓存（上限 100），将 MCP 工具转换为 Agent Terminal 的统一 Tool 接口：
...
| `src/entrypoints/mcp.ts` | MCP server 入口（Agent Terminal 作为 MCP server） |
