...
export function formatToolUseSummary(name: string, input: unknown): string {
  ...
  if (name === EXIT_PLAN_MODE_V2_TOOL_NAME) {
    return 'Review the plan in Agent Terminal on the web'
  }
    ...
    return (
      ...
      >
        <Box flexDirection="column" gap={1}>
          <Text dimColor>
            This will terminate the Agent Terminal on the web session.
          </Text>
          <Select
            options={[
    ...
    >
      <Box flexDirection="column" gap={1}>
        ...
        <Select
          options={[
            {
              label: 'Review in Agent Terminal on the web',
              value: 'open' as const,
            },
            ...(onKill && running
...
function ReviewSessionDetail({
  ...
  const options: { label: string; value: MenuAction }[] = completed
    ? [
        { label: 'Open in Agent Terminal on the web', value: 'open' },
