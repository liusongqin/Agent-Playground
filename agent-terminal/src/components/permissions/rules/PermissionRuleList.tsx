  ...
  return (
    <Box flexDirection="column" flexShrink={tab === 'allow' ? 0 : undefined}>
      <Text>
        {
          {
            allow: "Agent Terminal won't ask before using allowed tools.",
            ask: 'Agent Terminal will always ask for confirmation before using these tools.',
            deny: 'Agent Terminal will always reject requests to use denied tools.',
          }[tab]
        }
      </Text>
      <RulesTabContent
      ...
      <Pane color="permission">
        ...
        >
          ...
          <Tab id="workspace" title="Workspace">
            <Box flexDirection="column">
              <Text>
                Agent Terminal can read files in the workspace, and make edits when
