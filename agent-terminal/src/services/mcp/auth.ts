 ...
 *
 * Per RFC 7009, public clients (like Agent Terminal) should authenticate by including
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          if (code) {
            res.writeHead(200, { 'Content-Type': 'text/html' })
            res.end(
              `<h1>Authentication Successful</h1><p>You can close this window. Return to Agent Terminal.</p>`,
            )
            cleanup()
            resolveOnce(code)
          }
...
export class ClaudeAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `Agent Terminal (${this.serverName})`,
