...
export const ListPeersTool = buildTool({
  ...
  async description() {
    return 'Discover other Agent Terminal sessions for cross-session messaging'
  },
  async prompt() {
    return `List active Agent Terminal sessions that can receive messages via SendMessage.
