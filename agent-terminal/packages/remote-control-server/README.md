...
Remote Control Server 是 Agent Terminal 的远程控制后端，允许你通过浏览器 Web UI 远程监控和操作 Agent Terminal 会话。

## 功能

- **会话管理** — 创建、监控、归档 Agent Terminal 会话
- **实时消息流** — WebSocket / SSE 双向传输，实时查看对话和工具调用
- **权限审批** — 在 Web UI 中审批 Agent Terminal 的工具权限请求
...
## Agent Terminal 客户端配置

### 连接到自托管服务器

在 Agent Terminal 所在环境设置以下变量：
