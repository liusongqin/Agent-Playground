...
This is a **reverse-engineered / decompiled** version of Anthropic's official Agent Terminal CLI tool. The goal is to restore core functionality while trimming secondary capabilities. Many modules are stubbed or feature-flagged off. TypeScript strict mode is enforced（见 Working with This Codebase 段的 tsc 要求）。
...
| `packages/cc-knowledge/` | Agent Terminal 知识库（非 workspace 包） |
...
- **`src/services/acp/`** — ACP agent 实现，包含 `agent.ts`（AcpAgent 类）、`bridge.ts`（Agent Terminal ↔ ACP 桥接）、`permissions.ts`（权限处理）、`entry.ts`（入口）。
