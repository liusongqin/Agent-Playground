...
Remote Control Server 是 Agent Code 的远程控制后端，允许你通过浏览器 Web UI 远程监控和操作 Agent Code 会话。

## 功能

- **会话管理** — 创建、监控、归档 Agent Code 会话
- **实时消息流** — WebSocket / SSE 双向传输，实时查看对话和工具调用
- **权限审批** — 在 Web UI 中审批 Agent Code 的工具权限请求
