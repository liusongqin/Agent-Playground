...
description: "从源码角度解析 Agent Code 子 Agent：AgentTool.call() 的完整执行链路、Fork 子进程的 Prompt Cache 共享、Worktree 隔离、工具池独立组装、以及结果回传的数据格式。"
...
| **claude-code-guide** | 继承父模型 | 受限 | 解答 Agent Code 使用问题 |
