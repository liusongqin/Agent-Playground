...
本指南说明如何将 Remote Control Server (RCS) 部署到私有环境，并通过 Agent Code CLI 连接使用。

## 架构概览

```
┌──────────────────┐                    ┌──────────────────────┐
│  Agent Code CLI  │ ◄── HTTP/SSE/WS ─►│  Remote Control      │
...
- 一台可被 Agent Code CLI 和 Web 浏览器同时访问的服务器（物理机、VM、容器均可）
- [Docker](https://www.docker.com/)
- 启用 `BRIDGE_MODE` feature flag 的 Agent Code 构建
...
ACP session 在 Web UI 中显示品牌色标签，与普通 Agent Code session 区分。
...
└──────────────────────────────────────────────────────────┘
 ...
 1. Agent Code CLI 启动，设置环境变量指向自托管 RCS
