...
### 说明
通过 WebSocket 远程控制 Agent Code 会话。支持自托管私有部署。
...
### 说明
Daemon 模式允许 Agent Code 作为后台长驻进程运行，管理多个 worker。
...
### 说明
同一台机器上的多个 Agent Code 实例通过 UDS（Unix Domain Socket / Windows Named Pipe）自动发现并协作。首个启动的实例成为 main，后续自动注册为 sub。
