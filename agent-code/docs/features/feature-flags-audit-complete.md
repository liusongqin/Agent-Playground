# Agent Code 编译时特性标志（Feature Flags）完整审计报告
...
**功能描述**: 远程桥接模式。允许 Agent Code CLI 通过 WebSocket 连接到远程服务端（如 claude.ai Web 端），实现远程控制、会话转发、权限代理、附件传输等功能。这是 Agent Code 最大的子系统之一。
**分类**: COMPLETE
...
**编译时引用次数**: 32
**功能描述**: 协调器模式。允许 Agent Code 作为"领导者"协调多个"工作者"代理并行执行任务。工作者可以在同一进程内运行（in-process），也可以通过 tmux/iTerm2 面板运行。支持权限同步、重连、团队管理等。
**分类**: COMPLETE
...
**编译时引用次数**: 1
**功能描述**: CCR（Agent Code Remote）远程设置命令。
**分类**: COMPLETE
...
**功能描述**: Kairos 是 Agent Code 最大的功能集合。它是一个综合性平台功能，涵盖频道通知、主动模式、简报、GitHub Webhook、推送通知等多个子系统。几乎贯穿整个代码库。
**分类**: PARTIAL
...
**编译时引用次数**: 3
**功能描述**: 守护进程模式。允许 Agent Code 作为后台长驻 supervisor 进程运行，管理多个 worker。
**分类**: COMPLETE（已恢复）
**核心实现文件**:
...
*本文档由自动审计生成，基于对 Agent Code 源代码中所有 `feature('...')` 引用的穷举搜索。每个标志的引用次数包含单引号和双引号两种形式。*
